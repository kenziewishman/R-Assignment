---
title: "Wishman R Assignment Updated"
...
###Summary: fang_et_al_genotypes: 2782 rows, 986 columns
...
###Summary: snp_position: 983 rows, 15 columns
...
for (i in 1:length(maize_increasing$Chromosome)) {write.table(subset.data.frame(maize_increasing, maize_increasing$Chromosome == i), file = paste0("asc",maize_names[i], ".txt"), sep = "\t", row.names = F, quote = F)}
...
for (i in 1:length(maize_decreasing$Chromosome)) {write.table(subset.data.frame(maize_decreasing, maize_decreasing$Chromosome == i), file = paste("desc", maize_names[i], ".txt"), sep = "\t", row.names = F, quote = F)}
